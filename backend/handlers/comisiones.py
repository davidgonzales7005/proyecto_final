...
def handler(event, context):
    ...
    if method == "PATCH" and "distribuidor_id" in path and "periodo" in path:
        # actualiza porcentaje y/o total_ventas (recalcula si mandas ambos)
        exp = []
        names = {}
        vals = {}
        if "porcentaje" in body:
            exp.append("#p = :p")
            names["#p"] = "porcentaje"
            vals[":p"] = float(body["porcentaje"])
        if "total_ventas" in body:
            exp.append("#t = :t")
            names["#t"] = "total_ventas"
            vals[":t"] = float(body["total_ventas"])
        if "comision_calculada" in body:
            exp.append("#c = :c")
            names["#c"] = "comision_calculada"
            vals[":c"] = float(body["comision_calculada"])
